...
import { Prisma, Schedule, Clinic, Doctor } from "@prisma/client";
...
export class ScheduleServiceBase {
  ...
  async getClinic(parentId: string): Promise<Clinic | null> {
  ...
  async getDoctor(parentId: string): Promise<Doctor | null> {
    return this.prisma.schedule
      .findUnique({
        where: { id: parentId },
      })
